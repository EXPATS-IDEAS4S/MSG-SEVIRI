...
outpath = '/data1/crops/npy_crops_test/png/'
        ...
        for day in days:
            ...
            if my_obj is not None:
                ...
                for timestamp in timestamps:
                    ...
                    #if there are no Nan, the months is between the defined range and the hour is between the defined range  
                    if not is_all_nan_ds and not is_outside_range and month >= month_start and month <= month_end and hour >= hour_start and hour < hour_end:          
                        ...
                        #Plot the crops
                        # Extract variables
                        data = ds_time.IR_108.isel(time=0)  # Select first (only) time index
                        lons, lats = ds_time.lon, ds_time.lat
                        ...
                        # Add map features
                        ax.add_feature(cfeature.COASTLINE, linewidth=0.8, color="yellow")
                        ax.add_feature(cfeature.BORDERS, linestyle=":", color="yellow")
                        ax.add_feature(cfeature.LAND, edgecolor="yellow")#, facecolor="lightgray")

                        # Plot the data
                        c = ax.pcolormesh(lons, lats, data, cmap="Greys", transform=ccrs.PlateCarree())

                        # Add colorbar
                        #cbar = plt.colorbar(c, ax=ax, orientation="vertical", pad=0.02)
                        #cbar.set_label("IR_108 Temperature (K)")

                        # Title
                        #ax.set_title("IR_108 Brightness Temperature")

                        # Save the figure
                        plt.savefig(f'{outpath}{filename_to_save}.png', dpi=300, bbox_inches="tight")
