...
for modis_file in modis_files:
    ...
    if np.sum(~modis_cloud_masked.mask)<10:
    ...
    #print(cmsaf_lon_grid.shape, cmsaf_lat_grid.shape, cmsaf_cloud_mask.shape)
    #print(cmsaf_lon_masked.shape, cmsaf_lat_masked.shape, cmsaf_cloud_masked.shape)
    #print(binarized_modis_mask.data)
    #print(binarized_modis_mask.mask)#, modis_lat_masked.shape, modis_cloud_masked.shape)
    #print(modis_lon_masked.shape, modis_lat_masked.shape, cmsaf_regridded.shape)
    ...
    cbar = plt.colorbar(im, ax=axes[3], orientation='vertical', shrink=0.6, pad=0.05)
    ...
    for i, ax in enumerate(axes):
        # Set the extend
        ax.set_extent([lonmin_plot, lonmax_plot, latmin_plot, latmax_plot], crs=ccrs.PlateCarree())
    ...
    fig.savefig(f"{output_path}check_cmsaf_mask_regrid_modis_{modis_start_time:%Y%m%d%H%M}.png", bbox_inches='tight')
    plt.close()
    print('fig saved in', f"{output_path}check_cmsaf_mask_regrid_modis_{modis_start_time:%Y%m%d%H%M}.png")
